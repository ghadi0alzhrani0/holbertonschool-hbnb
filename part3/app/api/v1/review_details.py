...
class RatingList(Resource):
    ...
    def post(self):
        ...
        error = _review_access_error(
            (ratings_api.payload or {}).get("review_id")
        )
        if error:
            return error
        try:
...
class RatingResource(Resource):
    ...
    def put(self, object_id):
        ...
        details = facade.get_extended_resource("rating_details", object_id)
        if details is None:
            return {"error": "Rating details not found"}, 404
        error = _review_access_error(details.review.id)
        if error:
            return error
        return _update(
